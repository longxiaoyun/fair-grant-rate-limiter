...
A、B、C 三台机器共用一个令牌桶，**10s 窗口里最多发放 6 个令牌**。每拿到一个令牌，就可以执行一次任务。使用 `slidingWindow(10_000L, 6)` 配置这个窗口限制。

![A、B、C 都申请时各得两个令牌；只有 A、B 申请时各得三个，C 不申请也不预留令牌](docs/images/allocation.zh-CN.svg)
