...
评审日期：2026-09-21。实现基线为 PR #1 的 `587dcce`；本轮增加文档和场景测试，未改变令牌发放算法。

## 结论

**按资源共享令牌桶，再按存活等待节点分发令牌，这个通用架构适合保留。Kafka → ODPS 是解释它、验证它的真实案例，不应成为库的硬编码业务。**

当前实现可以作为公平令牌分发组件，但不能据“平均 5 次／秒”就验收为“已经保证 ODPS 每 15 秒最多 75 次 Commit”。本轮复现了默认配置的窗口反例。真实 SDK 调用边界和消费侧的数据可靠性，也不在已有令牌测试的证明范围内。
...
- 当前接入示例显式使用 burst=1 平滑发令牌，并明确不声称已完成服务端窗口验收。
- 在真实链路检查实际 Commit 发出/接收时序、其他写入者及重试，选择必要的速率余量。
- 若库要直接提供严格窗口功能，应增加通用的窗口时长和最大操作数约束，与公平队列原子组合。不能把 75、15 秒或 ODPS 判断写死进公共库。**本轮没有实现该功能。**
...
重新检查了公共 API、配置与 key 规则、三段 Lua、Redis 异常/回执路径、本地降级、现有测试、构建和 CI。令牌计费与等待队列仍是通用组件；本轮仅修正 Maven 项目描述的旧“最久未获准”表述，没有引入 Kafka 或 ODPS 依赖。
...
本地 Java 21、Redis 8.0.2 完整测试结果：**93 项，失败 0，错误 0，跳过 0**。原有 4 JVM／40 次 HTTP 调用端到端测试仍通过，但它也不是 Kafka → ODPS 生产端到端测试。
