...
| `slidingWindow(windowMs, maxPermits)` | Disabled | Positive duration/count; strict rolling grant limit, requires DENY |
...
| `WAIT` | false | No token, full window, another waiter, deny fallback or exhausted local share |
...
## Strict window and immediate execution

`.slidingWindow(15_000L, 75)` caps NEW grants in every `(t−15000ms, t]` interval at 75, in addition to the token bucket. Redis TIME has microsecond resolution; logical time never moves backwards. `window_full` returns WAIT. Records expire individually at the left boundary. Receipt replay never adds or extends a window record. Independent grant sequences prevent reused request IDs from overwriting history. Clearing pending work never refunds quota.

Strict windows require DENY; configuration rejects ALLOW/LOCAL_SHARE and local limiters reject window configuration. Redis state loss or a forward clock jump can still invalidate correspondence with real elapsed time.

`new FairGrantExecutor(limiter).tryExecute(resourceKey, clientId, action)` acquires fresh quota and invokes the callback once on the calling thread only when granted. Business exceptions propagate without refund; retry by invoking again to acquire fresh quota. It does not schedule, replay receipts, or retry operations. SDK-internal retries remain the application's responsibility.

## Guarantees and boundaries

- **Rate:** on the normal Redis path, an interval of `t` seconds admits approximately at most `burst + ratePerSec × t` new grants. This permits bursts. When enabled, the rolling window adds its configured cap on new grants.
...
1. Checks the existing bucket's rate, burst and window settings against the caller's configuration.
...
5. Removes expired window records and checks remaining window capacity.
6. Only if both budgets are available and this client is first, debits a token, appends a unique window record, writes a receipt and removes the waiting turn.
...
| `permit:<base64url(client)>:<base64url(request)>` | string + TTL | Redis grant time in microseconds |
...
The E2E case starts four JVMs sharing rate=20/burst=2 plus 6 grants per 500ms, performs 40 distinct HTTP commits and 40 acquisition receipt replays, verifies ten completions per client and initial FIFO fairness, and checks the token envelope and rolling-window cap across all grant timestamps.
...
`SlidingWindowTest` substitutes only the production Lua TIME read to test exact microsecond boundaries, individual expiry, dual budgets, receipt replay, FIFO and clock rollback. `SlidingWindowIT` repeats the contract on real Redis and adds production-clock checks: 30 concurrent nodes share 75 permits, every node progresses, grant 76 with rate=5/burst=5 cannot arrive before 15 seconds, and outages deny strict grants. These tests do not call production Kafka or ODPS.
